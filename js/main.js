...
// ── Support modal ────────────────────────
const SUPPORT_LIMIT_RUB = 50000;
const SUPPORT_PAYMENT_BASE_URL = 'https://all-bot.ru/pay/egypt-guide';
const SUPPORT_CONTACT_URL = 'mailto:info@all-bot.ru?subject=Поддержка Egypt Guide';

function parseSupportAmount(value) {
  const raw = String(value).trim().replace(/\s/g, '').replace(',', '.');
  if (!raw) return { value: null, reason: 'empty' };
  if (!/^\d+(?:\.\d{1,2})?$/.test(raw)) return { value: null, reason: 'invalid' };

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) return { value: null, reason: 'invalid' };
  if (parsed > SUPPORT_LIMIT_RUB) return { value: parsed, reason: 'large' };
  return { value: parsed, reason: null };
}

function formatSupportAmountForUrl(amount) {
...
function initSupportModal() {
  ...
  overlay.innerHTML = `
    <div class="support-modal" role="dialog" aria-modal="true" aria-labelledby="support-title">
      <button class="support-close" id="support-close" type="button" aria-label="Закрыть">×</button>
      <div class="support-ornament">☕</div>
      <p class="support-kicker">Проект «Египетская сила»</p>
      <h3 id="support-title">Угостить кофе</h3>
      <p class="support-lead">Egypt Guide остается открытым. Если гид помог, можно поддержать проект любой комфортной суммой — так мы будем обновлять советы, карту и полезные места.</p>
      <div class="support-amounts" role="group" aria-label="Выбрать сумму поддержки">
        <button class="support-amount-btn" type="button" data-support-amount="100">100 ₽</button>
        <button class="support-amount-btn" type="button" data-support-amount="500">500 ₽</button>
        <button class="support-amount-btn" type="button" data-support-amount="3000">3000 ₽</button>
      </div>
      <label class="support-custom-label" for="support-custom-amount">Своя сумма</label>
      <div class="support-custom-row">
        <input id="support-custom-amount" class="support-custom-input" type="text" inputmode="decimal" autocomplete="off" placeholder="Например, 700">
        <span class="support-ruble">₽</span>
      </div>
      <p class="support-message" id="support-message" aria-live="polite"></p>
      <p class="support-consent">Оплата пройдет на защищенной странице All-Bot.ru. Нажимая кнопку, вы соглашаетесь с <a href="https://all-bot.ru/offer" target="_blank" rel="noopener noreferrer">офертой</a>.</p>
      <div class="support-actions">
        <button class="support-pay-btn" id="support-pay-btn" type="button">Перейти к оплате</button>
        <a class="support-contact-btn" id="support-contact-btn" href="${SUPPORT_CONTACT_URL}">Написать нам</a>
      </div>
      <p class="support-legal">Добровольная оплата за цифровой гид Egypt Guide и развитие проекта.</p>
      <div class="support-links">
        <a href="https://all-bot.ru/offer" target="_blank" rel="noopener noreferrer">Оферта</a>
        <span>·</span>
        <a href="https://all-bot.ru/privacy" target="_blank" rel="noopener noreferrer">Политика</a>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);

  const closeBtn = overlay.querySelector('#support-close');
  const amountButtons = overlay.querySelectorAll('.support-amount-btn');
  const input = overlay.querySelector('#support-custom-amount');
  const message = overlay.querySelector('#support-message');
  const payBtn = overlay.querySelector('#support-pay-btn');
  const contactBtn = overlay.querySelector('#support-contact-btn');
  let selectedAmount = null;

  function setMessage(text = '', type = '') {
    message.textContent = text;
    message.className = type ? `support-message ${type}` : 'support-message';
  }
  ...
  function setSelectedAmount(amount) {
    ...
    payBtn.textContent = `Перейти к оплате ${amount} ₽`;
    contactBtn.classList.remove('is-visible');
    setMessage('');
  }

  function readCurrentAmount() {
    const parsed = input.value ? parseSupportAmount(input.value) : { value: selectedAmount, reason: selectedAmount ? null : 'empty' };
    if (parsed.reason === 'large') {
      payBtn.disabled = true;
      contactBtn.classList.add('is-visible');
      setMessage('Для такой суммы лучше оформить поддержку отдельно. Напишите нам на info@all-bot.ru — подберем корректный способ.', 'is-large');
      return null;
    }
    if (parsed.reason || !parsed.value) {
      payBtn.disabled = true;
      contactBtn.classList.remove('is-visible');
      setMessage('Введите сумму в рублях', 'is-error');
      return null;
    }

    payBtn.disabled = false;
    contactBtn.classList.remove('is-visible');
    payBtn.textContent = `Перейти к оплате ${formatSupportAmountForUrl(parsed.value)} ₽`;
    setMessage('');
    return parsed.value;
  }
  ...
  amountButtons.forEach(button => {
    button.addEventListener('click', () => setSelectedAmount(Number(button.dataset.supportAmount)));
  });
